...
1. Peer configuration. Map a stable server name to command or endpoint, arguments, environment allowlist, authorization context, and transport kind.
...
4. Legacy fallback. Use `initialize` only after an unrecognized error or bounded timeout. Cache the era for that transport peer.
