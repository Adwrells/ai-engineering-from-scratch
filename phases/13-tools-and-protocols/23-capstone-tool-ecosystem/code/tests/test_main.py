...
class ToolEcosystemTests(unittest.TestCase):
    ...
    def test_unknown_token_is_rejected(self) -> None:
        result = main.gateway_call("unknown", "arxiv_search", {"query": "agent"}, "a" * 32, "b" * 16)
    ...
    def test_read_only_actor_cannot_generate_report(self) -> None:
        result = main.gateway_call("tok_bob", "generate_report", {}, "a" * 32, "b" * 16)
        self.assertEqual(result["error"], "insufficient_scope")
        self.assertEqual(main.AUDIT[-1]["decision"], "403")

    def test_report_is_task_and_ui_shaped(self) -> None:
        result = main.gateway_call("tok_alice", "generate_report", {}, "a" * 32, "b" * 16)
        self.assertEqual(result["_meta"]["task"]["state"], "completed")
        self.assertEqual(result["content"][1]["uri"], "ui://report/current")
        self.assertIn("<!doctype html>", result["_html"])

    def test_delegation_span_uses_current_a2a_operation_name(self) -> None:
        main.gateway_call("tok_alice", "generate_report", {}, "a" * 32, "b" * 16)
