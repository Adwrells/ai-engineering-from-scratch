...
```mermaid
flowchart TD
  I[agent.invoke_agent] --> L1[llm.chat]
...
| MCP task polling | `tasks/get` then `tasks/result` | A task identifier and immediate completed result |
...
|---|---|---|---|
| Discovery | Static `TOOLS` list | MCP initialization and `tools/list` | Wire transcript and schema validation |
...
| Tasks | Immediate local result | Durable task store plus `tasks/get`, `tasks/result`, cancellation, and TTL | State-transition and recovery tests |
...
| 06-14 | MCP client/server boundaries, transports, resources, prompts, tasks, and Apps |
...
1. Alice can read and generate a report, while Bob's write-scoped call is denied.
2. Every local span in one orchestrator run shares one trace identifier and records parent span identifiers.
3. The report result contains text, a task-shaped completion record, and a `ui://` reference.
4. The delegated writer remains opaque because the orchestrator records only the boundary span.
5. No output claims a network connection, OAuth exchange, collector export, browser render, or sandbox execution occurred.
...
1. Replace the static tool list with a real MCP client initialization and `tools/list`.
2. Replace static tokens with an authorization server and protected resource validation.
3. Implement durable task state and test `tasks/get`, `tasks/result`, cancellation, timeout, and restart recovery.
...
4. Add a task store that survives a process restart. Prove a client can resume with `tasks/get` and obtain `tasks/result` exactly once.
...
| Task augmentation | "Long tool call" | A protocol lifecycle with durable identity, state, polling, result retrieval, and cancellation semantics |
...
- [MCP specification 2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25) for initialization, tools, tasks, authorization, and Apps-related protocol foundations.
- [MCP Tasks](https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks) for `tasks/get`, `tasks/result`, cancellation, and state semantics.
